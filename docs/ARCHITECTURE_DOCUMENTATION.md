...
# 🌐 Description complète de l'architecture Housy

L'application **Housy** repose sur une architecture full-stack moderne, modulaire et scalable, intégrant les meilleures technologies du web, de l'IA et du DevOps pour la gestion intelligente de projets de construction.
...
- **Sécurité :** Validation côté client, protection XSS
- **Optimisations :** Lazy loading, code splitting, virtualisation, cache, compression
- **Structure :** Pages, composants réutilisables, hooks personnalisés, services API

## 2. Backend (Serveur)
- **Langage :** TypeScript (Node.js 18)
- **Framework :** Express.js
- **ORM :** Drizzle (type-safe, migrations, requêtes optimisées)
- **Validation :** Zod (schémas stricts)
- **Sécurité :** JWT, bcrypt, helmet, cors, rate limiting, audit logging
- **Services :** Architecture modulaire (services métier, routes REST, middlewares)
- **API :** RESTful, 75+ endpoints, documentation TypeScript
- **Optimisations :** Cache Redis, monitoring, logs structurés, health checks

## 3. Base de données
- **SGBD :** PostgreSQL 15
- **Modélisation :** 18+ tables (users, projects, materials, estimations, transactions...)
- **ORM :** Drizzle (typage strict, migrations, index, relations, champs JSON)
- **Sauvegarde :** Backups automatisés, volumes persistants Docker
- **Performance :** Indexation, requêtes optimisées, ACID

## 4. Intégration Intelligence Artificielle (LLM)
- **Système hybride multi-modèles :**
  - Modèles locaux via Ollama (llama3, qwen, mistral, phi, deepseek-coder...)
  - APIs cloud : OpenAI (GPT-4o), Anthropic Claude, DeepSeek
- **Routage intelligent :** Sélection automatique du modèle selon la tâche, le rôle et la disponibilité
- **Enrichissement contextuel :** 19 fichiers JSON du marché tunisien injectés dans les prompts
- **Fallback automatique :** Cascade entre providers pour garantir la disponibilité
- **Contrôle d'accès :** Permissions par rôle, audit, monitoring IA
- **Endpoints IA :** /api/ai/chat, /api/estimation-ai/generate, /api/projects/:id/generate-estimation

## 5. Dockerisation & Orchestration
- **Déploiement :** Docker multi-containers (frontend, backend, PostgreSQL, Redis, Nginx)
- **Orchestration :** Docker Compose (réseaux isolés, health checks, volumes persistants)


## 6. Documentation technique
- **Documentation vivante :**
  - `ARCHITECTURE_DOCUMENTATION.md` (présent document)
  - `README.md`, `docs/`, guides d'installation, guides API
  - Diagrammes UML (classe, séquence, activité, composants)
  - Documentation des schémas de données (`shared/schema.ts`)
- **Mise à jour continue :** Documentation alignée sur l'évolution du code et des modules

---

**En résumé :**
Housy combine une stack React/TypeScript moderne, un backend Express robuste, une base PostgreSQL optimisée, une intégration IA avancée (LLM multi-providers), et une dockerisation professionnelle pour offrir une plateforme de gestion de projets de construction intelligente, sécurisée, performante et évolutive.

---

## 🔗 Communication et architecture globale

L’architecture de Housy repose sur une communication claire et sécurisée entre chaque couche du système. Le frontend (React) interagit avec le backend (Express) via des appels API RESTful (`/api/*`), utilisant le typage TypeScript pour garantir la cohérence des données. Le backend traite les requêtes, applique la logique métier à travers des services spécialisés, valide les données avec Zod, puis interroge la base PostgreSQL via Drizzle ORM pour lire ou modifier les informations. Les modules d’intelligence artificielle (LLM) sont intégrés côté backend et exposés via des endpoints dédiés, permettant au frontend d’accéder à des fonctionnalités avancées (chat, estimation, analyse) de façon transparente. L’ensemble des services (frontend, backend, base de données, IA, cache Redis) est orchestré par Docker Compose, assurant l’isolation, la scalabilité et la résilience de l’application. Cette architecture modulaire, documentée et typée, permet une évolution rapide, une maintenance facilitée et une sécurité renforcée sur toute la chaîne applicative.
