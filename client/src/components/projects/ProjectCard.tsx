...
const ProjectCard = ({ imageUrl, title, description, progress, status, date }: ProjectCardProps) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-neutral-200 hover:shadow-2xl transition-shadow duration-200">
      <div className="relative">
        ...
        <div className="absolute top-3 right-3">
          <EnhancedTooltip content={`${progress}% terminé`}>
            <div className="bg-white bg-opacity-80 rounded-full p-1 shadow">
              <div className="w-10 h-10 rounded-full flex items-center justify-center"
                   style={{
                     background: `conic-gradient(${getProgressBarColor(progress)} ${progress}%, #e5e7eb ${progress}% 100%)`
                   }}>
                <div className="bg-white w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold">
                  {progress}%
                </div>
              </div>
            </div>
      ...
      <div className="p-5">
        <h3 className="font-bold text-lg mb-1 truncate text-[#162032]">{title}</h3>
        ...
        <div className="flex justify-between items-center text-xs pt-2 border-t border-neutral-100 mt-2">
