...
export default function ClientPaymentsPage() {
  ...
  const mutation = useMutation({
    mutationFn: async (paymentId: string) => {
      // Simulate a payment confirmation API call
  ...
  const confirmPaymentMutation = useMutation({
    mutationFn: async (paymentId: string) => {
      // Simulation d'un appel API pour confirmer le paiement
      await new Promise((resolve) => setTimeout(resolve, 1500));
      
      // Mettre à jour le statut du paiement localement (simulation)
      ...
      return { paymentId, updatedPayments };
    },
    onSuccess: (data) => {
      // Simuler une notification de succès
      console.log('Paiement confirmé avec succès:', data.paymentId);
      queryClient.invalidateQueries({ queryKey: ['client-payments'] });
    },
    onError: (error) => {
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            ...
                            >
                              ...
                              {confirmPaymentMutation.isPending ? 'Confirmation...' : 'Confirmer le paiement'}
                            </Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Confirmez-vous avoir effectué le paiement de <strong>{payment.amount.toLocaleString()} TND</strong> 
                                pour la facture <strong>{payment.invoiceNumber}</strong> ?
                                <br /><br />
                                Cette action marquera le paiement comme effectué et ne pourra pas être annulée.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
