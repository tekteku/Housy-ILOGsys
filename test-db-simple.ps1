...
try {
    ...
    if ($psqlVersion) {
        ...
        Write-Host "   [ERROR] PostgreSQL client not found" -ForegroundColor Red
        $psqlAvailable = $false
    }
} catch {
    Write-Host "   [ERROR] PostgreSQL client (psql) not found in PATH" -ForegroundColor Red
    Write-Host "   [INFO] Install PostgreSQL client tools or use pgAdmin" -ForegroundColor Yellow
    $psqlAvailable = $false
}
...
try {
    ...
    if ($connection.TcpTestSucceeded) {
        Write-Host "   [SUCCESS] Port $Port is accessible on $Host" -ForegroundColor Green
        $portAccessible = $true
    } else {
        Write-Host "   [ERROR] Port $Port is not accessible on $Host" -ForegroundColor Red
        Write-Host "   [INFO] Make sure PostgreSQL/Docker container is running" -ForegroundColor Yellow
        $portAccessible = $false
    }
} catch {
    Write-Host "   [ERROR] Cannot test port connectivity" -ForegroundColor Red
    $portAccessible = $false
}
...
if ($psqlAvailable -and $portAccessible) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "   [SUCCESS] Connected to database '$Database'" -ForegroundColor Green
            ...
            if ($adminUser) {
                Write-Host "   [INFO] Admin user found: $adminUser" -ForegroundColor Cyan
            } else {
                Write-Host "   [WARNING] No admin user found" -ForegroundColor Yellow
            }
            ...
            Write-Host "   [ERROR] Failed to connect to database '$Database'" -ForegroundColor Red
            Write-Host "   [INFO] Check database name, credentials, and server status" -ForegroundColor Yellow
        }
    } catch {
    ...
    Write-Host "3. Skipping database connection test" -ForegroundColor Yellow
    if (-not $psqlAvailable) {
        Write-Host "   [INFO] psql not available" -ForegroundColor Yellow
    }
    if (-not $portAccessible) {
        Write-Host "   [INFO] Port not accessible" -ForegroundColor Yellow
    }
}
