...
router.get('/kpis', async (req, res) => {
  try {
    ...
    // Statistiques sur les projets actifs
    const activeProjects = allProjects.filter((p: any) => p.status === 'active' || p.status === 'in_progress').length;
    const delayedProjects = allProjects.filter((p: any) => {
      if (p.status === 'completed') return false;
      const currentDate = new Date();
      const plannedEnd = new Date(p.endDate);
      return currentDate > plannedEnd;
    }).length;
    ...
    const kpisData = {
      ...
      financials: {
        ...
        averageProjectDuration: calculateAverageProjectDuration(allProjects),
        resourceUtilization: 85, // Placeholder - would be calculated from actual resource data
    ...
    res.json(createResponse(
      true,
      "Indicateurs clés de performance récupérés avec succès",
      kpisData
    ));
  } catch (error) {
...
// Fonction helper pour calculer la durée moyenne des projets
function calculateAverageProjectDuration(projects: any[]): number {
  ...
  const totalDuration = completedProjects.reduce((sum: number, p: any) => {
    ...
    const duration = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)); // days
    return sum + duration;
  }, 0);
...
// GET /api/analytics/extended/dashboard - Tableau de bord étendu avec nouvelles tables
router.get('/extended/dashboard', async (req, res) => {
  try {
    const { period = '30days' } = req.query as AnalyticsRequest;
    const { startDate, endDate } = calculateDateRange(period);
    ...
    const [
      ...
      db.select({ count: count() }).from(activeProjects),
      db.select({ count: count() }).from(clientRequests),
      db.select({ count: count() }).from(quotations),
      db.select({ count: count() }).from(payments),
      db.select().from(enhancedNotifications)
        .where(gte(enhancedNotifications.createdAt, startDate))
        .orderBy(desc(enhancedNotifications.createdAt))
        .limit(10)
    ]);

    // Statistiques par statut
    const [requestsByStatus, quotationsByStatus, projectsByStatus, paymentsByStatus] = await Promise.all([
      storage.getAllClientRequests(),
      storage.getAllQuotations(),
      storage.getAllActiveProjects(),
      storage.getPaymentsByProject()
    ]);

    const requestStats = requestsByStatus.reduce((acc: any, req: any) => {
      acc[req.status] = (acc[req.status] || 0) + 1;
      return acc;
    }, {});

    const quotationStats = quotationsByStatus.reduce((acc: any, quote: any) => {
      acc[quote.status] = (acc[quote.status] || 0) + 1;
      return acc;
    }, {});

    const projectStats = projectsByStatus.reduce((acc: any, proj: any) => {
      acc[proj.status] = (acc[proj.status] || 0) + 1;
      return acc;
    }, {});

    const paymentStats = paymentsByStatus.reduce((acc: any, pay: any) => {
    ...
    const pendingPayments = paymentsByStatus
      .filter((p: any) => p.status === 'pending')
      .reduce((sum: number, p: any) => sum + parseFloat(p.amount?.toString() || '0'), 0);

    const extendedDashboard = {
      summary: {
        totalActiveProjects: totalActiveProjects[0]?.count || 0,
        totalClientRequests: totalClientRequests[0]?.count || 0,
      ...
      statusBreakdown: {
        clientRequests: requestStats,
        quotations: quotationStats,
        activeProjects: projectStats,
        payments: paymentStats
      },
      recentActivity: recentNotifications.slice(0, 5),
      conversionRates: {
        requestToQuotation: requestsByStatus.length > 0 
          ? Math.round((quotationsByStatus.length / requestsByStatus.length) * 100) 
          : 0,
        quotationToProject: quotationsByStatus.length > 0 
          ? Math.round((projectsByStatus.length / quotationsByStatus.length) * 100) 
          : 0
      },
      period,
      dateRange: {
    ...
    res.json(createResponse(true, "Tableau de bord étendu récupéré avec succès", extendedDashboard));
  } catch (error) {
    console.error('Erreur lors de la génération du tableau de bord étendu:', error);
    res.status(500).json(createResponse(
      false,
      "Erreur lors de la génération du tableau de bord étendu",
      undefined,
      error instanceof Error ? error.message : 'Erreur inconnue'
    ));
  }
...
// GET /api/analytics/extended/workflow - Analyse du workflow complet
router.get('/extended/workflow', async (req, res) => {
  try {
    const { period = '6months' } = req.query as AnalyticsRequest;
    const { startDate, endDate } = calculateDateRange(period);

    // Récupérer les données du workflow
    const [allRequests, allQuotations, allProjects, allPayments] = await Promise.all([
      storage.getAllClientRequests(),
      storage.getAllQuotations(),
      storage.getAllActiveProjects(),
    ...
    // Analyser le pipeline de conversion
    const workflowAnalysis = {
      totalRequests: allRequests.length,
      approvedRequests: allRequests.filter(r => r.status === 'approved').length,
      quotationsSent: allQuotations.filter(q => q.status === 'sent').length,
      quotationsAccepted: allQuotations.filter(q => q.status === 'accepted').length,
      activeProjects: allProjects.filter(p => ['planning', 'in_progress'].includes(p.status)).length,
      completedProjects: allProjects.filter(p => p.status === 'completed').length,
      
      conversionRates: {
        requestApprovalRate: allRequests.length > 0 
          ? Math.round((allRequests.filter(r => r.status === 'approved').length / allRequests.length) * 100) 
          : 0,
        quotationAcceptanceRate: allQuotations.length > 0 
          ? Math.round((allQuotations.filter(q => q.status === 'accepted').length / allQuotations.length) * 100) 
          : 0,
        projectCompletionRate: allProjects.length > 0 
          ? Math.round((allProjects.filter(p => p.status === 'completed').length / allProjects.length) * 100) 
          : 0
      },

      averageTimeInStages: {
        requestToQuotation: calculateAverageStageTime(allRequests, allQuotations, 'createdAt', 'createdAt'),
        quotationToProject: calculateAverageStageTime(allQuotations, allProjects, 'sentAt', 'startDate'),
        projectDuration: calculateAverageProjectDuration(allProjects)
      },      financialMetrics: {
        averageQuotationValue: allQuotations.length > 0 
          ? Math.round(allQuotations.reduce((sum, q) => sum + (parseFloat(q.finalAmount?.toString() || '0') || 0), 0) / allQuotations.length)
          : 0,
        averageProjectValue: allProjects.length > 0 
          ? Math.round(allProjects.reduce((sum, p) => sum + (parseFloat(p.contractValue?.toString() || '0') || 0), 0) / allProjects.length)
          : 0,        totalRevenue: allPayments
          .filter(p => p.status === 'completed')
          .reduce((sum, p) => sum + parseFloat(p.amount?.toString() || '0'), 0)
      }
    };

    res.json(createResponse(true, "Analyse du workflow récupérée avec succès", workflowAnalysis));
  } catch (error) {
    console.error('Erreur lors de l\'analyse du workflow:', error);
    res.status(500).json(createResponse(
      false,
      "Erreur lors de l'analyse du workflow",
      undefined,
      error instanceof Error ? error.message : 'Erreur inconnue'
    ));
  }
...
// GET /api/analytics/extended/categories - Analyse par catégories de projets
router.get('/extended/categories', async (req, res) => {
  try {
    const [categories, requests, projects, allQuotations, allRequests] = await Promise.all([
      storage.getActiveProjectCategories(),
      storage.getAllClientRequests(),
      storage.getAllActiveProjects(),
      storage.getAllQuotations(),
      storage.getAllClientRequests()
    ]);const categoryAnalysis = categories.map(category => {
      const categoryRequests = requests.filter(r => r.categoryId === category.id);
      // Note: activeProjects don't have categoryId directly, we need to get it through quotation/request relationship
      const categoryProjects = projects.filter(p => {
        // Find the quotation for this project and check its request's categoryId
        const quotation = allQuotations.find(q => q.id === p.quotationId);
        if (quotation) {
          const request = allRequests.find(req => req.id === quotation.requestId);
          return request && request.categoryId === category.id;
        }
        return false;
      });
      ...
      return {
        ...
        metrics: {
          ...
          conversionRate: categoryRequests.length > 0 
            ? Math.round((categoryProjects.length / categoryRequests.length) * 100) 
            : 0,
          averageProjectValue: categoryProjects.length > 0 
            ? Math.round(categoryProjects.reduce((sum, p) => sum + (parseFloat(p.contractValue?.toString() || '0') || 0), 0) / categoryProjects.length)
            : 0,
          totalRevenue: categoryProjects.reduce((sum, p) => sum + (parseFloat(p.contractValue?.toString() || '0') || 0), 0),
          completedProjects: categoryProjects.filter(p => p.status === 'completed').length
        }
      };
    });

    // Tri par revenus décroissants
    categoryAnalysis.sort((a, b) => b.metrics.totalRevenue - a.metrics.totalRevenue);

    res.json(createResponse(true, "Analyse par catégories récupérée avec succès", categoryAnalysis));
  } catch (error) {
    console.error('Erreur lors de l\'analyse par catégories:', error);
    res.status(500).json(createResponse(
      false,
      "Erreur lors de l'analyse par catégories",
      undefined,
      error instanceof Error ? error.message : 'Erreur inconnue'
    ));
  }
...
// Fonction helper pour calculer le temps moyen entre deux étapes
function calculateAverageStageTime(sourceData: any[], targetData: any[], sourceTimeField: string, targetTimeField: string): number {
  const validPairs = [];
  
  for (const source of sourceData) {
    const matchingTarget = targetData.find(t => t.clientRequestId === source.id || t.quotationId === source.id);
    if (matchingTarget && source[sourceTimeField] && matchingTarget[targetTimeField]) {
      const sourceTime = new Date(source[sourceTimeField]).getTime();
      const targetTime = new Date(matchingTarget[targetTimeField]).getTime();
      if (targetTime > sourceTime) {
        validPairs.push(Math.ceil((targetTime - sourceTime) / (1000 * 60 * 60 * 24))); // jours
      }
    }
  }
  
  return validPairs.length > 0 
    ? Math.round(validPairs.reduce((sum, days) => sum + days, 0) / validPairs.length)
    : 0;
}
