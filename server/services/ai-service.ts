...
Note technique : Modèles IA temporairement indisponibles (${errors.join("; ")})`;
  ...
  private async processChatWithOllama(chatHistory: any[], systemMessage: string): Promise<string> {
    try {
      ...
      // Updated priority: prefer qwen2.5-coder for coding tasks, then llama3.1 and phi
      const preferredModels = ["qwen2.5-coder:latest", "llama3.1:latest", "llama3.1:8b", "phi:latest", "deepseek-coder:latest", "llama3.1", "llama3", "mistral", "phi", "llama2", "gemma", "neural-chat", "orca-mini"];
      let modelToUse: OllamaModel = "phi"; // Default fallback
      
      // Find first preferred model that's available (with exact match including tags)
